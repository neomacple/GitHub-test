...
from cross_validation import dc_model
...
### MACCS FINGERPRINT ###
train_fp = [smiles_to_ecfp4(x, fp_size) for x in df_train['Canonical SMILES']]
val_fp = [smiles_to_ecfp4(x, fp_size) for x in df_val['Canonical SMILES']]
...
with open(logs, 'w') as log:
    ...
    for n_split in sample_splits:
        for n_leaf in sample_leaf:
            scores = cv_decision_tree(5, (X_train, y_train), n_split, n_leaf, seed)
            train_accs = []
            test_accs = []
            outs += f"\nmin_sample_split: {n_split:2d}, "
            outs += f"min_sample_leaf: {n_leaf:2d}\n"
            ...
            print(f"min_sample_split: {n_split:2d}, min_sample_leaf: {n_leaf:2d}, \
    ...
    model = dc_model(depth=None, min_split=min_sample_split, \
